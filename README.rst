...
Because mrcrypt follows the AWS Encryption SDK's message format, files encrypted by mrcrypt can also be decrypted by the AWS Encryption SDK for Java. See the section titled `Compatability with the AWS Encryption SDK <#compatability-with-the-aws-encryption-sdk>`__ for details.
